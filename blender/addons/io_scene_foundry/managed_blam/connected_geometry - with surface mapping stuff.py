...
class SurfaceMapping:
    def __init__(self, first: int, count: int, surface: 'CollisionSurface', surf_to_tri_block: TagFieldBlock):
        ...
        for i in range(first, first + count):
            result = (self.byteswarp(surf_to_tri_block.Elements[i].Fields[0].Data) + 1) / 3 - 1
            if not result.is_integer():
                print(result, int(result))
            self.triangle_indices.append(int(result))
        
    def byteswarp(self, value: int):
        """
        Decodes the annoying way Bungie decided to store triangle mapping in Reach.
        Bit-shifts `value` left by `shift` bits, encodes into little-endian bytes,
        then reads those bytes back left-to-right (big-endian) and returns the int.
        """
        shifted = value << 12
        # Convert to hex string without '0x', padded to even length
        hex_str = f"{shifted:x}"
        if len(hex_str) % 2:
            hex_str = '0' + hex_str
        # Split into big-endian byte pairs
        pairs = [hex_str[i:i+2] for i in range(0, len(hex_str), 2)]
        # Trim trailing zero-byte pairs
        while pairs and pairs[-1] == '00':
            pairs.pop()

        total = 0
        last_index = len(pairs) - 1
        for idx, pair in enumerate(pairs):
            # interpret each pair as a hex integer
            dec_val = int(pair, 16)
            if idx < last_index:
                if dec_val > 0:
                    total += dec_val * 256
        ...
        if not utils.is_corinth() and not for_cinematic:
            ...
            if self.has_collision:
                ...
                if not self.collision_is_proxy:
                    ...
                    self.surface_triangle_mapping = [SurfaceMapping(e.Fields[0].Data, e.Fields[1].Data, self.collision_info.surfaces[e.ElementIndex], surf_to_tri_block) for e in element.SelectField("Block:surfaces").Elements if e.Fields[1].Data > 0]
...
class Mesh:
    ...
    def _create_mesh(self, name, parent, nodes, subpart: MeshSubpart | None, parent_bone=None, local_matrix=None, is_io=False, surface_triangle_mapping=[]):
        ...
        if surface_triangle_mapping:
            print("Mesh {ob.name} has triangle mapping!")
            ...
            has_render_only = mapping_count != len(mesh.polygons)
            if any_slip_surface or any_ladder or any_breakable or has_render_only:
                bm = bmesh.new()
                bm.from_mesh(mesh)
                if any_slip_surface:
        ...
        # Setting two sidedness here for collision purposes
