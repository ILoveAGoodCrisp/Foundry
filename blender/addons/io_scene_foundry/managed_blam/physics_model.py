...
from .connected_geometry import CollisionMaterial, Hinge, LimitedHinge, NodeEdge, Ragdoll, RigidBody
from .. import utils
from . import Tag
...
class PhysicsTag(Tag):
    ...
    def to_blend_objects(self, collection: bpy.types.Collection, edit_armature=None, markers=True):
            ...
            for permutation_element in region_element.Fields[1].Elements:
                ...
                for rigid_element in permutation_element.SelectField("rigid bodies").Elements:
                    ...
                    if body_index > -1 and self.block_rigid_bodies.Elements.Count > body_index:
                        body = RigidBody(self.block_rigid_bodies.Elements[body_index], region, permutation, materials, four_vectors_offset, self)
                        four_vectors_offset = body.four_vectors_offset
                        rigid_objects = body.to_objects()
                        for ob in rigid_objects:
                            bone = nodes[body.node_index]
                            world = ob.matrix_world.copy()
                            ob.parent = armature
                            # world = edit_armature.matrices[bone]
                            ob.parent_type = 'BONE'
                            ob.parent_bone = bone
                            ob.matrix_world = world
                            # ob.matrix_local = Matrix.Translation([0, edit_armature.lengths[bone], 0]).inverted()
        ...
        for element in self.block_hinge_constraints.Elements:
            ...
            ob = hinge.to_object()
            self._parent_constraint(hinge, ob, armature, edit_armature)
            collection.objects.link(ob)
            objects.append(ob)
        ...
        for element in self.block_limited_hinge_constraints.Elements:
            ...
            self._parent_constraint(limited_hinge, ob, armature, edit_armature)
            collection.objects.link(ob)
            objects.append(ob)
        ...
        for element in self.block_ragdoll_constraints.Elements:
            ...
            self._parent_constraint(ragdoll, ob, armature, edit_armature)
            collection.objects.link(ob)
            objects.append(ob)
            
        return objects
            
    def _parent_constraint(self, constraint: Hinge, ob: bpy.types.Object, armature, edit_armature):
        ...
        ob.parent_bone = bone
        ob.matrix_world = world
        ob.matrix_local = Matrix.Translation([0, edit_armature.lengths[bone], 0]).inverted() @ constraint.matrix_a # @ Matrix.LocRotScale(Vector(constraint.center) * 100, Euler((0,0,0)), Vector.Fill(3, 1))