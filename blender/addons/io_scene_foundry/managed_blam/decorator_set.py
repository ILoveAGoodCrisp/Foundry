    ...
    def to_blender(self, collection, get_material=False, always_extract_bitmaps=False):
        ...
        if base and Path(base).exists():
            ...
            with RenderModelTag(path=base) as base_render:
                base_obs = base_render.to_blend_objects(collection, True, False, base_collection, no_armature=True)
                for ob in base_obs:
        ...
        if lod2 and Path(lod2).exists():
            ...
            with RenderModelTag(path=lod2) as lod2_render:
                lod2_obs = lod2_render.to_blend_objects(collection, True, False, lod2_collection, no_armature=True)
                for ob in lod2_obs:
        ...
        if lod3 and Path(lod3).exists():
            ...
            with RenderModelTag(path=lod3) as lod3_render:
                lod3_obs = lod3_render.to_blend_objects(collection, True, False, lod3_collection, no_armature=True)
                for ob in lod3_obs:
        ...
        if lod4 and Path(lod4).exists():
            ...
            with RenderModelTag(path=lod4) as lod4_render:
                lod4_obs = lod4_render.to_blend_objects(collection, True, False, lod4_collection, no_armature=True)
                for ob in lod4_obs:
